...
#Effective field
H = H_eff(S0, 0, J, dz, mu, B)
#Parameters to use in the LLG below
param = (gamma, mu, a, H)

#Solutions
#Numerical:
S, t = integrator(Tmax, S0[0], t0, h, LLG, param)
...
#     #Heun
#     S, t = integrator(Tmax, S0[0], t0, h, LLG, param)
#     error = np.abs(S[-1,0] -  Sx * np.cos(gamma * B0 * t[-1]))
#     err_Heun = np.append(err_Heun, error)
#     #Euler
#     S, t = integrator(Tmax, S0[0], t0, h, LLG, param, method = euler)
#     error = np.abs(S[-1,0] -  Sx * np.cos(gamma * B0 * t[-1]))
...
# plt.plot(steps, 10**-3 * steps**2, '--', label = "$\sim h^2$", color = 'k') #Prefactor chosen to lie close to Heun line
# plt.xscale("log")
# plt.yscale("log")
# plt.xlabel("Timestep")
...
#Damping
a = 0.05
param = (gamma, mu, a, H) #Parameters to use in the LLG below
Tmax = 50
...
#x-component
plt.figure(4)
plt.title("Damped Oscillation of $S_{x}$")
...
#y-component
plt.figure(5)
plt.title("Damped Oscillation of $S_{y}$")
